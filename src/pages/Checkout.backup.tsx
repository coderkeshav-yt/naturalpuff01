...
const Checkout = () => {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (paymentMethod === 'online') {
        try {
          console.log('Creating Razorpay order for order ID:', order.id);
          
          // Create Razorpay order via edge function
          const { data: razorpayOrderData, error: rzpError } = await supabase.functions.invoke('createRazorpayOrder', {
            body: {
          ...
          // Handle Razorpay order creation errors
          if (rzpError) {
            console.error('Razorpay order creation error:', rzpError);
            throw new Error(`Failed to create Razorpay order: ${rzpError.message || 'Unknown error'}`);
          }
          
          if (!razorpayOrderData || !razorpayOrderData.data || !razorpayOrderData.data.id) {
          ...
          const razorpayOrder = razorpayOrderData.data;
          ...
          const razorpayKey = import.meta.env.VITE_RAZORPAY_KEY_ID;
          if (!razorpayKey) {
            throw new Error('Razorpay API key is not configured. Please check your environment variables.');
          }
          ...
          const options = {
            ...
            handler: function(response: any) {
              ...
              // Use the dedicated payment success handler
              handlePaymentSuccess({
                razorpay_payment_id: response.razorpay_payment_id,
                razorpay_order_id: response.razorpay_order_id,
                razorpay_signature: response.razorpay_signature,
            ...
            modal: {
              ondismiss: function() {
                ...
                toast({
                  title: 'Payment Cancelled',
                  description: 'You closed the payment window. Your order has been saved and you can try again later.',
                });
                navigate(`/order-success?id=${order.id}&payment=pending`);
              }
            }
          };
          
          // Create Razorpay instance
          console.log('Creating Razorpay instance with order ID:', razorpayOrder.id);
          const razorpay = createRazorpayInstance(options);
          ...
          console.log('Opening Razorpay checkout...');
          razorpay.open();
          return; // Don't set isProcessing to false yet as payment is in progress
        } catch (razorpayError: any) {
          console.error('Razorpay error:', razorpayError);
          toast({
            title: 'Order Created',
            description: 'Your order has been created, but there was an issue with the payment system. You can complete the payment later.',
            variant: 'default'
          });
